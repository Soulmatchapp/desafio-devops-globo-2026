...
sequenceDiagram
    ...
    GHA->>GHA: build da imagem Docker (py-service e/ou go-service)
    GHA->>AR: docker push imagem:SHA
    GHA->>TF: terraform apply (var py_image/go_image = :SHA)
...
1. Push na branch `main` com mudança em `apps/py-service/**` ou `apps/go-service/**`.
2. GitHub Actions builda a imagem só do serviço que mudou (matrix job) e publica no Artifact Registry com a tag = SHA do commit.
3. `terraform apply` atualiza a variável de imagem do Cloud Run correspondente → Cloud Run cria uma **nova revisão** automaticamente.
...
1. Push com mudança em `terraform/**` (ex: mudar TTL de cache, adicionar Cloud Armor).
